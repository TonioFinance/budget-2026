...
# --- STYLE OBSIDIAN & AZURE (SAAS EDITION + MOTION) ---
st.markdown("""
    <style>
    ...
    /* --- GLOBAL ANIMATIONS (0.1s for smoothness) --- */
    * { transition: all 0.1s ease-out; }
    ...
    .cat-amount { color: #FFFFFF !important; font-size: 17px; font-weight: 700; text-shadow: 0 0 15px rgba(255, 255, 255, 0.4); }
    ...
    .trans-amount { color: #FFFFFF !important; font-weight: 800; font-size: 16px; text-shadow: 0 0 10px rgba(255,255,255,0.2); }
...
def get_progress_html(name, reel, prevu):
    ...
    # Logic color: Green -> Orange (2/3) -> Red (100%)
...
if row_history_start != -1:
    for i in range(row_history_start, len(all_rows)):
        ...
        if len(row) > 4 and str(row[0]).strip() not in ["", "nan"]:
            ...
            raw_expenses.append({"Date": row[0], "Merchant": row[1], "Amount": amt, "Category": row[4]})

# --- TABS SYSTEM ---
tab_dash, tab_scan = st.tabs(["📊 DASHBOARD", "📸 SCAN RECEIPT"])
...
with tab_dash:
    ...
    else: insight_html = f"<div class='insight-banner insight-green'><i class='ph ph-check-circle'></i> On track</div>"
    ...
    with col_c1:
        ...
        # Smart Sort: Active first, then by Planned amount
    ...
    st.markdown("<h3 style='color: #FFFFFF; font-size: 20px; text-align: center;'><i class='ph ph-trend-up'></i> Spending Trend</h3>", unsafe_allow_html=True)
    if raw_expenses:
        df_trends = pd.DataFrame(raw_expenses)
        df_trends['Date'] = pd.to_datetime(df_trends['Date'])
        daily_spend = df_trends.groupby('Date')['Amount'].sum().reset_index().sort_values('Date')
        daily_spend['Cumulative'] = daily_spend['Amount'].cumsum()

        fig_trend = go.Figure()
        fig_trend.add_trace(go.Scatter(x=daily_spend['Date'], y=daily_spend['Cumulative'], mode='lines', fill='tozeroy', 
                                      line=dict(color='#3B82F6', width=4), fillcolor='rgba(59, 130, 246, 0.1)'))
        fig_trend.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=300, 
                                margin=dict(t=10, b=10, l=10, r=10), xaxis=dict(showgrid=False, color="#64748B"), 
...
with tab_scan:
    ...
    st.markdown("<p style='color: #94A3B8;'>Take a photo of your receipt to auto-fill the expense.</p>", unsafe_allow_html=True)
    
    uploaded_file = st.file_uploader("", type=["jpg", "png", "pdf"])
    
    if uploaded_file:
        st.markdown("<div class='scanner-dropzone'>Scanning receipt... <br> (AI Engine Simulation)</div>", unsafe_allow_html=True)
        # Simulation UI
        col1, col2 = st.columns(2)
        with col1:
            scanned_merchant = st.text_input("Detected Merchant", value="Store (Simulated)")
            scanned_amount = st.number_input("Detected Amount", value=15.00)
        with col2:
            scanned_cat = st.selectbox("Detected Category", list(form_cat_map.keys()), key="scanned_cat")
            st.write("<br>", unsafe_allow_html=True)
            if st.button("SAVE SCANNED EXPENSE", use_container_width=True):
                st.success("Transaction added!")
    else:
        st.markdown("""<div class="scanner-dropzone"><i class="ph ph-cloud-arrow-up" style="font-size: 48px; color: #3B82F6;"></i><br><br>Drop receipt here or click to upload</div>""", unsafe_allow_html=True)
