...
with tab_dashboard:
    ...
    if raw_expenses:
        ...
        # Force flexible date parsing to safely catch user's expenses dates
        df_trends['Date'] = pd.to_datetime(df_trends['Date'], format='mixed', dayfirst=True, errors='coerce')
        df_trends = df_trends.dropna(subset=['Date'])
        
        if not df_trends.empty:
            daily = df_trends.groupby('Date')['Amount'].sum().reset_index().sort_values('Date')
            
            # Cumulative logic maintained to show the "mountain" filling up to the budget limit
            ...
            fig.add_trace(go.Scatter(
                ...
                fill='tozeroy', 
                name='Actual Spend', 
                line=dict(color='#60A5FA', width=4, shape='spline'), 
