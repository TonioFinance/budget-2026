...
st.markdown("""
    ...
    * { transition: all 0.05s ease-out; }
    ...
    /* --- CUSTOM TABS NAVIGATION (PREMIUM DESIGN) --- */
    ...
    /* Injection des pictogrammes Phosphor via CSS */
...
if row_history_start != -1:
    for i in range(row_history_start, len(all_rows)):
        ...
        if len(row) > 4 and str(row[0]).strip() not in ["", "nan"]:
            ...
            expenses_list.append({"Date": row[0], "Merchant": row[1], "Montant": format_chf(raw_amt) + " CHF", "Catégorie": row[4], "RawAmount": raw_amt})

# --- TABS SYSTEM ---
tab_dash, tab_scan = st.tabs(["Dashboard", "Scanner"])
...
with tab_dash:
    ...
    if expenses_list:
        ...
        if not df_trends.empty:
            daily = df_trends.groupby('Date')['RawAmount'].sum().reset_index().sort_values('Date')
            daily['Cumulative'] = daily['RawAmount'].cumsum()
